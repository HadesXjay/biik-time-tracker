...
export default function Dashboard() {
  ...
  // 🔗 UPDATED: Pointing to your latest API
    ...
    <div className="min-h-screen bg-[#121212] text-white p-6 font-sans">
      <div className="max-w-4xl mx-auto flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          {/* 🛠️ SECRET ADMIN DOOR: Only shows for you */}
          {userEmail === "jayvimp@gmail.com" && (
            <button 
              onClick={() => window.location.href = "/admin"}
              className="text-[10px] bg-blue-600/20 text-blue-400 border border-blue-500/30 px-3 py-1 rounded-md hover:bg-blue-600/40 font-bold uppercase"
            >
              Admin Console
            </button>
          )}
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <div className="bg-[#1e1e1e] p-8 rounded-3xl border border-[#333] flex flex-col items-center">
          <input 
            ...
            className="bg-transparent text-center text-lg mb-4 outline-none border-b border-[#333] w-full pb-2 focus:border-white transition-colors" 
          />
          <div className="text-6xl font-mono font-black mb-8 tracking-tighter">{formatTime(seconds)}</div>
          <div className="flex gap-3 w-full">
            <button 
              onClick={toggleTimer} 
              className={`flex-1 py-4 rounded-2xl font-bold transition-all ${isActive ? "bg-yellow-600" : "bg-white text-black hover:bg-gray-100"}`}
            >
              {isActive ? "PAUSE" : "START"}
            </button>
            {!isActive && seconds > 0 && (
              <button onClick={handleFinish} className="flex-1 py-4 rounded-2xl font-bold bg-green-600 hover:bg-green-700">
                FINISH
              </button>
            )}
          </div>
        ...
        <div className="bg-[#1e1e1e] p-8 rounded-3xl border border-[#333]">
          <h2 className="text-[10px] text-gray-500 uppercase tracking-widest mb-6 font-bold">Weekly Performance (Hrs)</h2>
            ...
            <tbody className="divide-y divide-[#333]">
              {tasks.length > 0 ? tasks.slice().reverse().map((t: any, i) => (
                <tr key={i} className="hover:bg-[#252525] transition-colors">
                  ...
                  <td className="px-8 py-5 text-gray-600 text-right font-mono">{t.date.split('T')[0]}</td>
                </tr>
              )) : (
                <tr>
