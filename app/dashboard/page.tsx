    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8 font-sans">
      <div className="max-w-5xl mx-auto flex justify-between items-center mb-10">
        <div>
          ...
          <p className="text-[10px] text-blue-500 font-mono uppercase tracking-widest">Master: jayvimp@gmail.com</p>
        </div>
        <button onClick={() => window.location.href = "/dashboard"} className="text-xs bg-[#1e1e1e] px-4 py-2 rounded-lg border border-[#333] hover:bg-[#252525]">Back to Dashboard</button>
      </div>

      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6 px-2">
          <h2 className="font-bold text-lg">System Feed</h2>
          <button onClick={refreshLogs} className="text-[10px] text-gray-500 hover:text-white uppercase tracking-widest">{loading ? "Syncing..." : "↻ Refresh Database"}</button>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="text-sm divide-y divide-[#1e1e1e]">
              {tasks.length > 0 ? tasks.slice().reverse().map((t, i) => (
                <tr key={t.taskId || i} className="hover:bg-[#1a1a1a] transition-colors">
                ...
                <tr><td colSpan={4} className="px-8 py-20 text-center text-gray-700 italic">{loading ? "Accessing Google Sheets..." : "No logs found."}</td></tr>
              )}
            </tbody>
          </table>
        </div>
