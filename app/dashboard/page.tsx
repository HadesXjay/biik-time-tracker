...
export default function Dashboard() {
  ...
  const [weeklyStats, setWeeklyStats] = useState({ Mon: 0, Tue: 0, Wed: 0, Thu: 0, Fri: 0 })
  ...
  useEffect(() => {
    ...
    else { setUserEmail(email); refreshData(email); }
  }, []);
  ...
  const handleFinish = async () => {
    ...
    setSeconds(0); setTaskName(""); refreshData(userEmail);
  };
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white p-6">
      <div className="max-w-4xl mx-auto flex justify-between items-center mb-8">
        <h1 className="text-xl font-bold">🐷 Biik Tracker</h1>
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <div className="bg-[#1e1e1e] p-8 rounded-3xl border border-[#333] flex flex-col items-center">
          ...
          <div className="flex gap-3 w-full">
            <button onClick={toggleTimer} className={`flex-1 py-4 rounded-xl font-bold transition-all ${isActive ? "bg-yellow-600" : "bg-white text-black"}`}>
              {isActive ? "PAUSE" : "START"}
            </button>
            {!isActive && seconds > 0 && <button onClick={handleFinish} className="flex-1 py-4 rounded-xl font-bold bg-green-600">FINISH</button>}
          </div>
        </div>
